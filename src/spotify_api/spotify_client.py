...
class SpotifyPlaylistUpdater:
    ...
    def get_playlist_tracks(self, playlist_name: str) -> List[Dict]:
        ...
        try:
            ...
            result = self._call_spotify(self.sp.playlist_items, playlist_id, limit=100)
            total = int(result.get("total") or 0)
    ...
    def find_non_artist_tracks(
        ...
        for i in range(0, len(track_ids), batch_size):
            ...
            tracks_details = self._call_spotify(self.sp.tracks, batch_ids)
            for j, full_track in enumerate(tracks_details.get("tracks") or []):
                ...
                if target_artist_id in track_artist_ids:
                    continue
                original_track = playlist_tracks[i + j]
                main_artist = None
    ...
    def find_non_artist_tracks_multiple(
        ...
        for i in range(0, len(track_ids), batch_size):
            ...
            for j, full_track in enumerate(tracks_details.get("tracks") or []):
                ...
                original_track = playlist_tracks[i + j]
                original_artist_names = {(a.get("name") or "").strip().lower() for a in (original_track.get("artists") or [])}
                original_artist_ids = {a.get("id") for a in (original_track.get("artists") or []) if a.get("id")}

                is_allowed = bool(original_artist_ids.intersection(target_artist_ids)) or bool(original_artist_names.intersection(normalized_names))
                if is_allowed:
                    continue
    ...
    def get_artist_all_tracks(self, artist_name: str) -> List[Dict]:
        ...
        try:
            ...
            for i in range(0, len(all_album_ids), batch_size):
                ...
                albums_details = self._call_spotify(self.sp.albums, batch_ids)
                for album_detail in albums_details.get("albums") or []:
                    ...
                    for track in ((album_detail.get("tracks") or {}).get("items") or []):
                        ...
                        # Include tracks where the target artist is one of the artists.
                        if not (
                            any(a.get("id") == artist_id for a in track_artists)
                            or any((a.get("name") or "") == artist_name_found for a in track_artists)
                        ):
                            continue
                        all_tracks.append(
                            {
    ...
    def analyze_all_playlists(self) -> Dict:
        ...
        try:
            ...
            for i, playlist in enumerate(playlists, 1):
                ...
                logger.info("Analyzing playlist %s/%s: %s", i, total_playlists, playlist_name)
                ...
                try:
                    ...
                    if not artist_name:
                        logger.info("Playlist '%s' has no name, skipping", playlist_name)
                        continue
                    
                    # Get artist tracks
                    artist_tracks = self.get_artist_all_tracks(artist_name)
                    if not artist_tracks:
                        logger.info("No tracks found for artist '%s', skipping", artist_name)
                    ...
                    if not playlist_tracks:
                        logger.info("Playlist '%s' is empty, skipping", playlist_name)
                        results[playlist_name] = {"missing": [], "extra": [], "total_tracks": 0}
                        continue
                    
                    logger.info(
                        "Found %s artist tracks and %s playlist tracks",
                        len(artist_tracks),
                        len(playlist_tracks),
                    )
                    ...
                    logger.info("Missing: %s, Extra: %s", len(unique_missing), len(extra_tracks))
                    
                except Exception as e:
                    logger.exception("Error analyzing playlist '%s'", playlist_name)
